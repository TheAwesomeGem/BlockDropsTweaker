 ...
 */
public class BlockNbtRemoveCommand extends BlockDropsBaseCommand
{
    @Override
    ...
    protected void onCommand(MinecraftServer server, EntityPlayer player, String[] args) throws CommandException
    {
        ...
        {
            player.sendMessage(ChatUtil.getNormalMessage("That nbt does not exist."));

            return;
        }

        blockDropData.nbtList.remove(nbt);

        player.sendMessage(ChatUtil.getNormalMessage("Removed NBT '" + nbt + "'"));
    }
    ...
    public List<String> getTabCompletions(MinecraftServer server, ICommandSender sender, String[] args, @Nullable BlockPos targetPos)
    {
        ...
        if(args.length == 1)
        {
            if(sender instanceof EntityPlayer)
            {
                ...
                if (blockDropData != null)
                {
                    tabList.addAll(getListOfStringsMatchingLastWord(args, blockDropData.nbtList));
                }
            }
        }
