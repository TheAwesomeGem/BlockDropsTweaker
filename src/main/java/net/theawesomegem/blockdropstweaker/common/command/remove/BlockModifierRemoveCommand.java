 ...
 */
public class BlockModifierRemoveCommand extends BlockDropsBaseCommand
{
    @Override
    ...
    protected void onCommand(MinecraftServer server, EntityPlayer player, String[] args) throws CommandException
    {
        ...
        if(!blockDropData.modifierList.contains(modifier))
        {
        ...
        blockDropData.modifierList.remove(modifier);

        player.sendMessage(ChatUtil.getNormalMessage("Removed Modifier '" + modifier + "'"));
    }
    ...
    public List<String> getTabCompletions(MinecraftServer server, ICommandSender sender, String[] args, @Nullable BlockPos targetPos)
    {
        ...
        if(args.length == 1)
        {
            if(sender instanceof EntityPlayer)
            {
                ...
                if (blockDropData != null)
                {
                    tabList.addAll(getListOfStringsMatchingLastWord(args, blockDropData.modifierList));
                }
            }
        }
